...
from models import ProductShoppingListBase, ProductShoppingListCreate, ShoppingListRead, ShoppingListCreate, ShoppingListUpdate, ProductShoppingListRead
...
@router.get("/{id}/product/{product_id}", response_model=ShoppingListRead, responses={404: {"description": "The item was not found"}})
...
@router.post("/{id}/product", response_model=ShoppingListRead, status_code=status.HTTP_201_CREATED)
...
async def update_shopping_list_product(id: int, product_id: int, product: ProductShoppingListBase, db: Session = Depends(get_session)):
